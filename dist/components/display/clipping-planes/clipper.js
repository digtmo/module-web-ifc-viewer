import { Vector3, Matrix3, Box3Helper } from 'three';
import { IfcComponent } from '../../../base-types';
import { IfcPlane } from './planes';
export class IfcClipper extends IfcComponent {
    constructor(context, ifc) {
        super(context);
        this.orthogonalY = true; ////// en false no vi nada 
        this.toleranceOrthogonalY = 1.7;
        this.planeSize = 10; //// estaba en 5 quedó en 10 
        this.createPlane = () => {
            if (!this.enabled)
                return;
           ...
           /*  ///////////////// triple clic en los planos  //////////////////  */


            const xPlane = this.createFromNormalAndCoplanarPoint(
                new Vector3(1, 0, 0),
                new Vector3(this.planeSize  + -10, 0, 0)
              );
              const yPlane = this.createFromNormalAndCoplanarPoint(
                new Vector3(0, 1, 0),
                new Vector3(0, this.planeSize -10, 0)
              );
              const zPlane = this.createFromNormalAndCoplanarPoint(
                new Vector3(0, 0, 1),
                new Vector3(0, 0, this.planeSize -10)
              ); 
        ...
        this.createPlaneFromIntersection = (intersection) => {
            var _a;
            const constant = intersection.point.distanceTo(new Vector3(1, 0, 0));
            ...
            this.context.addClippingPlane(plane.plane);
            this.updateMaterials();
            const boxHelper = new Box3Helper(intersection.object.geometry.boundingBox);  /// ------------ box helper --------------- 
    ...
    Object.values(this.ifc.loader.ifcManager.subsets.getAllSubsets()).forEach((subset) => {
        ...
        if (mesh.material) {
            ...
            if (mesh.geometry.boundingBox) {
                ...
                // ------------------ Box Helper cada vez que se crea un plano nuevo ----------------
       /*          const boxHelper = new Box3Helper(mesh.userData.wireframe);
